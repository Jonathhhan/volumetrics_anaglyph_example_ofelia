...
public:
    ...
    void loadVolumeData(unsigned char * data, int w, int h, int d, int xOffset, int yOffset, int zOffset)
    {
        volumetrics.loadVolumeData(data, w, h, d, xOffset, yOffset, zOffset);
    }
...
public:
    ...
    void beginLeft()
    {
        cameraAnaglyph.beginLeft();
    }

    void endLeft()
    {
        cameraAnaglyph.endLeft();
    }

    void beginRight()
    {
        cameraAnaglyph.beginRight();
    }

    void endRight()
    {
        cameraAnaglyph.endRight();
    }

    void endStereo()
    {
        cameraAnaglyph.endStereo();
    }

    bool isStereo()
    {
        return cameraAnaglyph.isStereo();
    }

    bool isMono()
    {
        return cameraAnaglyph.isMono();
    }

    void enableStereo()
    {
        cameraAnaglyph.enableStereo();
    }

    void disableStereo()
    {
        cameraAnaglyph.disableStereo();
    }

    bool isColorMaskEnabled()
    {
        return cameraAnaglyph.isColorMaskEnabled();
    }

    void enableColorMask()
    {
        cameraAnaglyph.enableColorMask();
    }

    void disableColorMask()
    {
        cameraAnaglyph.disableColorMask();
    }

    void toggleColorMask()
    {
        cameraAnaglyph.toggleColorMask();
    }

    void setPosition(float x, float y, float z)
    {
        cameraAnaglyph.setPosition(x, y, z);
    }

    void lookAt(ofVec3f x)
    {
        cameraAnaglyph.lookAt(x);
    }

    void eyeSeparation(float x)
    {  
        cameraAnaglyph.eyeSeparation = x;
    }

    void focalLength(float x)
    {
        cameraAnaglyph.focalLength = x;
    }

    void orbitDeg(float x, float y, float z)
    {
